...
class AuthService {
  ...
  isTokenExpired(token: string) {
    ...
    try {
      ...
      // Check if the decoded token has an 'exp' (expiration) property and if it is less than the current time in seconds. Divide by 1000 to convert milliseconds to seconds
